...
public class BrightnessTrait extends Trait {
	...
	private BrightnessTraitWantListener brightnessTraitWantListener;

	public BrightnessTrait(Device device, boolean commandOnlyBrightness, BrightnessTraitWantListener brightnessTraitWantListener) {
		...
		this.brightnessTraitWantListener = brightnessTraitWantListener;
	}
	...
	public CompletableFuture<Void> wantBrightness(String requestId, int brightness) {
	...
	public CompletableFuture<Void> executeCommand(String requestId, String command, JsonNode params) {
		if ("action.devices.commands.BrightnessAbsolute".equals(command))
			return wantBrightness(requestId, params.get("brightness").asInt());

		if ("action.devices.commands.BrightnessRelative".equals(command))
			return wantBrightness(requestId, getBrightness() + params.get("brightnessRelativePercent").asInt());
