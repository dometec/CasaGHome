 ...
 * Max 1 call/sec for the same Device and Trait.
 */
public class DefaultReportStateAndNotification {
	 ...
	 */
	public DefaultReportStateAndNotification(ScheduledExecutorService scheduledExecutor, int msDebounce) {
	...
	private void _sendReportStateAndNotification(Device device, Optional<Trait> trait, Optional<Notification> notification) {
		...
		HomeGraphService homegraphService = CasaGHomeSetup.getHomegraphService();
		...
		try {
			...
			trait.ifPresent(t -> status.putAll(t.getStatus()));
