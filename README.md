...
This library models devices and traits exposed from Google Home. It implements callbacks to execution from Google Home and sends status and notifications to Google Home.
...
To manage your devices via Google Home you need to set up an Authentication Server and create Google Home Project.

### Account linking

To receive Google Home API calls you need to set up the Account Linking. This allows Google Home to obtain an Access Token and a Refresh Token from your OAuth2 Identity Provider Server and to use this token (access token) to call your service on behalf of your user.

I use Keycloak to set up a my IdP Server, but you can use any OAuth2.0 server or services ([here](https://www.osys.it/googlehome/#/4) a presentation, in Italian, with some screenshots).

For more info see [here](https://developers.home.google.com/cloud-to-cloud/project/authorization).

Eventually, you need these parameters from OAuth2 platform to create the Google Home Project:
...
You have to create a ReST Endpoint (with ssl) where Google Home can send its requests. This library helps to decode and manage these requests.
...
In the [example](https://github.com/dometec/CasaGHome-examples) I use a [Quarkus](https://quarkus.io/) (in red) web application and an instance of [Keycloak](http://keycloak.jboss.org/) started on my machine and [ngrok](https://ngrok.com/) to route requests from a public endpoint to my local service.

![Demo](MiaPiattaformaIOTDemo.jpg)

If you want to simplify the setup and avoid to install Keycloak, I include in the example [here](https://github.com/dometec/CasaGHome-examples) a *fake* authentication, use it only as a showcase.

## How to use this library

This library is used inside a backend that receive http requests from Google Home to synchronize, query or execute commands to your device/IoT platform/etc. (in red in the picture above) and is able also to call Google Home to report state and notifications (or alerts) from your devices/platform.
...
- CasaGHome: Responsible to manage requests from Google Home and to callback your code to execute actions.
- CasaReportStateAndNotification: To send Device change / notification or alert to Google Home.

The CasaGHome need only an ObjectMapper object, while CasaReportStateAndNotification needs also an instance of [HomeGraphService](https://googleapis.dev/java/google-api-services-homegraph/latest/com/google/api/services/homegraph/v1/HomeGraphService.html), a ScheduledExecutorService and a integer that represents, in millisecond, the time to wait betweent two identical update requests (for the same device and trait). These last two parameters are used to debounce call towards Google Home.
...
The library models [devices](https://developers.home.google.com/cloud-to-cloud/guides) that can be used with Google Home, like light, garage, sensor, ecc. 

You have to model all devices that a single User can manage into a _Casa_ Object. Each user has an instance of _Casa_ that contains all its devices.
...
You can store this data locally or on any external service like DB, Redis, Mongo, ecc. For example, in the example I store them in memory:
...
Let's start with a simple Light that we can switch on or off and that Google Home can query for its status:
...
We define a Device with its ID, NAME and zero o more optional Traits. We put it in a UserDevices that represents all User's devices and save it elsewhere (I use an in-memory store like defined before).

The [OnOffTrait](https://developers.home.google.com/cloud-to-cloud/traits/onoff) has two attributes:
 - commandOnlyOnOff
 - queryOnlyOnOff
...
You need to provide an endpoint to Google Home where it can send requests:
The following example is a standard ReST endpoint using Jakarta JaxRS. The only two things it needs are the user id and the UserDevices storage of this user. 
```java
...
How can we receive commands from Google Home? You have to define callbacks on CasaGHome object instance.
...
Whene you receive a command, you have to act on your device / IoT platform to do the command. If you receive an ACK from device in field you can wait this to resolve the completable future, or you can complete it immediately like in the following snippet:
```java
casaGHome.setOnOffTraitWantListener(new OnOffTraitWantListener() {
	...
	public CompletableFuture<Void> onWantOnOff(Device device, String requertId, boolean on) {
		...
		// When we have an ACK from the field we can complete the
		// CompletableFuture
		cf.complete(null);
		return cf;
	}
...
Please note that you have to send ReportState not only when the state of your device changes autonomously in the field (es: someone switch on the light at home) but also when the change cames from Google Home itself (es: a user switch on the light from Google Home app), see [here](https://developers.home.google.com/cloud-to-cloud/integration/report-state) (from Docs: ***Note: Subsequent state information returned via EXECUTE and QUERY intents will not be stored in Home Graph. It is your responsibility to call Report State to update Home Graph with the latest state information.***)
